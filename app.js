...
function setAgentLogLines(lines = []) {
  if (!agentLogEl) return;
  agentLogEl.innerHTML = lines
...
function renderBoard() {
  ...
  boardEl.dataset.size = state.size;
...
function readPlayerState() {
  const position = serializePosition(state.player);
  const status = getCurrentGameStatus();
...
async function executeSolution(path) {
  const rawMoves = Array.isArray(path) ? path : [];
  ...
  if (!moves.length) {
    ...
    return {
      success: false,
      escaped: Boolean(finalStatus.escaped),
      won: Boolean(finalStatus.won),
      moves: finalStatus.moves,
      finalPlayerPosition: finalStatus.player,
      path: [],
      results,
      status: finalStatus.status,
      message: 'No moves were provided to execute.',
    };
  }
  ...
  for (let index = 0; index < moves.length; index += 1) {
    ...
    if (state.won || getCurrentGameStatus() === 'escaped' || state.lives === 0 || state.timeRemaining === 0) {
      stopReason = 'game-finished';
      break;
    }
    ...
    setAgentLogLines([
      ...
      'Finding solution... ✓',
      `Executing solution...`,
      `Move ${moveNumber}/${totalMoves} ${direction === 'up' ? '↑' : direction === 'down' ? '↓' : direction === 'left' ? '←' : '→'}`,
    ]);

    const result = movePlayerHandler({ direction });
    const playerState = getPlayerState();
    const verifiedPosition = playerState.position || serializePosition(state.player);
    const samePosition = result.player && verifiedPosition && result.player.row === verifiedPosition.row && result.player.col === verifiedPosition.col;

    const outcome = {
      ...result,
      verified: Boolean(result.success && samePosition),
      playerState,
      finalPlayerPosition: verifiedPosition,
    };
    results.push(outcome);

    if (!result.success || !samePosition || state.won || getCurrentGameStatus() === 'escaped') {
      stopReason = !result.success ? 'move-failed' : 'goal-or-exit';
      break;
    }
  ...
  const finalStatus = getGameStatus();
  const escaped = Boolean(finalStatus.escaped || finalStatus.won || state.won);

  setAgentLogLines([
    'AI Agent',
    'Analyzing maze... ✓',
    'Finding solution... ✓',
    escaped ? 'Maze escaped! ✓' : 'Execution finished.',
  ]);

  return {
    success: !stopReason || escaped,
    escaped,
    won: Boolean(finalStatus.won),
    moves: finalStatus.moves,
    finalPlayerPosition: finalStatus.player,
    ...
    status: finalStatus.status,
    stopReason,
    message: escaped ? 'Maze escaped successfully.' : stopReason === 'move-failed' ? 'Execution stopped because a move failed.' : 'Execution finished.',
  };
}
...
async function registerWebMCPTools() {
  ...
  const tools = [
    ...
    },
    {
      ...
      handler: async (input) => {
        const result = movePlayerHandler(input);
        await delay(400);
        return result;
      },
    },
    ...
    },
    {
      ...
      description: 'Execute a previously computed route on the live maze. Use this after find_solution when the user asks the agent to actually play the game.',
      inputSchema: {
        type: 'object',
        properties: {
          path: { type: 'array', items: { type: 'string', enum: ['up', 'down', 'left', 'right'] } },
        },
        required: ['path'],
        additionalProperties: false,
      },
      outputSchema: {
        type: 'object',
        properties: { success: { type: 'boolean' }, path: { type: 'array', items: { type: 'string' } }, status: { type: 'string' } },
        required: ['success', 'path', 'status'],
        additionalProperties: false,
      },
      handler: ({ path = [] } = {}) => Promise.resolve(executeSolution(path)),
    },
  ];
...
// -----------------------------------------------------------------------------
// Amazon Bedrock agent bridge
// -----------------------------------------------------------------------------
// Bedrock runs server-side. It requests tools; this browser executes the same
// live game functions that are also registered with WebMCP. This keeps the AI
// and WebMCP on the same game state without exposing AWS credentials.
...
const agentToolHandlers = {
  get_maze_state: () => window.getMazeState(),
  get_player_state: () => window.getPlayerState(),
  get_available_moves: () => window.getAvailableMoves(),
  move_player: (input) => window.gameAPI.movePlayer(input || {}),
  restart_game: () => window.restartGame(),
  get_game_status: () => window.getGameStatus(),
  find_solution: () => window.findSolution(),
  execute_solution: (input) => window.executeSolution(input?.path || []),
};

function setAgentActivity(message) {
  if (agentLogEl) agentLogEl.textContent = message;
}

function getTextFromBedrockContent(content = []) {
  return content.filter((block) => block.text).map((block) => block.text).join('\n').trim();
}

async function runBedrockAgent(userPrompt) {
  if (!userPrompt.trim()) return;

  agentRunBtn.disabled = true;
  setAgentActivity('Connecting to Amazon Bedrock...');

  const messages = [{ role: 'user', content: [{ text: userPrompt.trim() }] }];
  const maxTurns = 12;
  ...
  try {
    for (let turn = 0; turn < maxTurns; turn += 1) {
      ...
      if (!response.ok) throw new Error(payload.error || 'Bedrock request failed.');

      const assistantContent = payload.content || [];
      messages.push({ role: 'assistant', content: assistantContent });

      const toolUses = assistantContent.filter((block) => block.toolUse);
      if (!toolUses.length) {
        const finalText = getTextFromBedrockContent(assistantContent) || 'Agent finished.';
        setAgentActivity(finalText);
        return;
      }
      ...
      for (const block of toolUses) {
        ...
        const handler = agentToolHandlers[call.name];
        if (!handler) {
          toolResults.push({ toolResult: { toolUseId: call.toolUseId, status: 'error', content: [{ json: { error: `Unknown tool: ${call.name}` } }] } });
          continue;
        }

        setAgentActivity(`AI calling tool: ${call.name}`);
        let result;
        try {
        ...
        // Let the browser paint the move before the next Bedrock turn.
    ...
    throw new Error('Agent stopped after the safety limit of 12 turns.');
  } catch (error) {
    console.error('[Bedrock Agent]', error);
    setAgentActivity(`Agent error: ${error instanceof Error ? error.message : String(error)}`);
  } finally {
    agentRunBtn.disabled = false;
  }
}

agentRunBtn?.addEventListener('click', () => runBedrockAgent(agentPromptEl?.value || ''));
agentClearBtn?.addEventListener('click', () => {
  if (agentPromptEl) agentPromptEl.value = '';
  setAgentActivity('Agent ready. Enter a task above.');
});
agentPromptEl?.addEventListener('keydown', (event) => {
  if (event.key === 'Enter' && !event.shiftKey) {
    event.preventDefault();
    runBedrockAgent(agentPromptEl.value);
  }
});
